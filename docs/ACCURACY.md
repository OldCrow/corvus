...
(`tools/gen_*_reference.py`). CAVEAT (2026-08-21, #13): the writers convert
with `float(mpf)`, which rounds the mantissa to 53 bits and then again onto
the subnormal grid, so about 1% of subnormal-result rows (6/642 erfc,
9/931 exp_dd) are 1 ulp off correctly rounded; every "CR" or "0 ULP"
statement about a subnormal band below is relative to that reference
until it is regenerated. Reference
