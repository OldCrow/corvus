// Benchmark: corvus::exp (SIMD batch) vs scalar std::exp, ns per element.
// Not a correctness test; run manually on a quiet machine, Release build only.
//
//   ./build/tests/bench_exp
//
// Inputs are uniform in [-700, 700] — the full finite-result range, short of
...
int main() {
  ...
  for (size_t n : {1000UL, 10000UL, 100000UL, 1000000UL}) {
    ...
    const double simd_exp = NsPerElement(
        [&] {
          corvus::exp(in, out);
          g_sink = out[n / 2];
        },
    ...
    const double scalar_exp = NsPerElement(
        [&] {
    ...
    std::printf("%10zu %4s %14.2f %14.2f %9.2fx\n", n, "exp", simd_exp,
                scalar_exp, scalar_exp / simd_exp);
  }
  return 0;
}
